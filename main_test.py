...
class Car:
    ...
    def update_blocked_routes(self, _route):
        self.blocked_routes.add(_route)

    def clean_blocked_routes(self):
        self.blocked_routes = set()
    ...
    def closest_route(self):
        ...
        for _route in routes_list:
            if not _route.done and _route not in self.blocked_routes:
...
def find_closest_route(car):
    return car.closest_route()


def car_spreading(_spreading, _car_list):
    global routes_list
    busy_cars = False
    for car in _car_list:
        if car.available() and not car.status:
            route, min_time = car.closest_route()
            if route is not None:
                if _spreading.get(route) is None:
                    _spreading[route] = (car, min_time)
                    car.change_status()
                else:
                    busy_cars = True
                    other_car, other_min_time = _spreading[route]
                    if min_time > other_min_time:
                        _spreading[route] = (car, min_time)
                        car.change_status()
                        other_car.change_status()
                        other_car.update_blocked_routes(route)
                    else:
                        car.update_blocked_routes(route)
            else:
                car.change_status()
    if busy_cars:
        car_spreading(_spreading, _car_list)
    return _spreading
...
if __name__ == "__main__":
    ...
    for file_name in input_names:
        ...
        for current_time in range(total_time):
            spreading = {}
            spreading = car_spreading(spreading, car_list)
            for _route, _car in spreading.items():
                _route.done = True
                _car[0].add_route(_route, _car[1])
                _car[0].clean_blocked_routes()
                _car[0].change_status()
